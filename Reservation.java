...
public class Main {
	public static void main(String[] args) {
        ...
        // Ilipat ang declaration ng date, pax, name, contact, at email
        // sa labas ng loops para maging accessible sa buong reservation process.
        String name = "";
        String contact = "";
        String email = "";
        String date = "";
		...
		do {
			...
			case "1":
				...
				while (!backToMain) {
					...
					// Dito magsisimula ang loop para sa pagpili ng kuwarto, para kung mag-back, dito siya babalik.
					...
					while(!returnToRoomChoice) {
						...
						while (!validRoom) {
							...
							// Kung `B` ang input, babalik sa "Start reservation" screen
						...
						// Dito magsisimula ang loop para sa pagpili ng petsa
							...
							while (true) {
								...
								while (!validDay) {
									...
									if ((" " + days + " ").contains(" " + day + " ") && !unavailableDates.contains(roomName + " " + day + " of " + monthName)) {
                                        ...
                                        // Assign value to global `date` variable
                                        date = monthName + " " + day + " " + selectedWeekday;
                                        // Assign pax limit to global `pax` variable
                                        if (roomName.equals("NORMAL ROOM")) {
                                            pax = 4;
                                        } else if (roomName.equals("STANDARD ROOM")) {
                                            pax = 6;
								...
								if (backToMonth) {
									break; // Bumalik sa pagpili ng buwan
								}
								...
								if (backFromDetails) {
									continue; // Babalik sa pagpili ng araw
								}
